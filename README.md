...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String CREATE_COURSE_TABLE = "CREATE TABLE " + Config.TABLE_COURSE + "("
                ...
                + Config.COLUMN_COURSE_NAME + " TEXT NOT NULL, "
                + Config.COLUMN_COURSE_CODE + " INTEGER NOT NULL, "
                + Config.COLUMN_COURSE_CREDIT + " REAL, " //nullable
                ...
                + "CONSTRAINT " + Config.STUDENT_SUB_CONSTRAINT + " UNIQUE (" + Config.COLUMN_REGISTRATION_NUMBER + "," + Config.COLUMN_COURSE_CODE + ")"
                + ")";
...
public class DatabaseQueryClass {
    ...
    public long insertSubject(Subject subject, long registrationNo){
        ...
        ContentValues contentValues = new ContentValues();
        contentValues.put(Config.COLUMN_COURSE_NAME, subject.getName());
        contentValues.put(Config.COLUMN_COURSE_CODE, subject.getCode());
        contentValues.put(Config.COLUMN_COURSE_CREDIT, subject.getCredit());
        contentValues.put(Config.COLUMN_REGISTRATION_NUMBER, registrationNo);

        try {
            rowId = sqLiteDatabase.insertOrThrow(Config.TABLE_COURSE, null, contentValues);
        } catch (SQLiteException e){
            Logger.d(e);
        } finally {
            sqLiteDatabase.close();
    ...
    public Subject getSubjectById(long subjectId){
        ...
        Cursor cursor = null;
        try{
            cursor = sqLiteDatabase.query(Config.TABLE_COURSE, null,
                    Config.COLUMN_COURSE_ID + " = ? ", new String[] {String.valueOf(subjectId)},
                    null, null, null);

            if(cursor!=null && cursor.moveToFirst()){
                String subjectName = cursor.getString(cursor.getColumnIndex(Config.COLUMN_COURSE_NAME));
                int subjectCode = cursor.getInt(cursor.getColumnIndex(Config.COLUMN_COURSE_CODE));
                double subjectCredit = cursor.getDouble(cursor.getColumnIndex(Config.COLUMN_COURSE_CREDIT));
    ...
    public long updateSubjectInfo(Subject subject){
        ...
        try {
            rowCount = sqLiteDatabase.update(Config.TABLE_COURSE, contentValues,
    ...
    public List<Subject> getAllSubjectsByRegNo(long registrationNo){
        ...
        Cursor cursor = null;
        try{
            cursor = sqLiteDatabase.query(Config.TABLE_COURSE,
                    new String[] {Config.COLUMN_COURSE_ID, Config.COLUMN_COURSE_NAME, Config.COLUMN_COURSE_CODE, Config.COLUMN_COURSE_CREDIT},
    ...
    public boolean deleteSubjectById(long subjectId) {
        ...
        int row = sqLiteDatabase.delete(Config.TABLE_COURSE,
                Config.COLUMN_COURSE_ID + " = ? ", new String[]{String.valueOf(subjectId)});
    ...
    public long getNumberOfSubject(){
        ...
        try {
            count = DatabaseUtils.queryNumEntries(sqLiteDatabase, Config.TABLE_COURSE);
        } catch (SQLiteException e){
            Logger.d("Exception: " + e.getMessage());
            Toast.makeText(context, e.getMessage(), Toast.LENGTH_LONG).show();
        } finally {
